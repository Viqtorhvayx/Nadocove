...
import { usePortfolioHistory } from "@/lib/use-portfolio-history";

const CHART_WIDTH = 700;
const CHART_HEIGHT = 200;
const PADDING_Y = 12;

const PERIODS: { period: IndexerPortfolioPeriod; label: string }[] = [
  { period: "day", label: "1D" },
  { period: "week", label: "1W" },
  { period: "month", label: "1M" },
  { period: "allTime", label: "Max" },
];

/**
 * The dashboard's hero chart — same real portfolio-value history
 * (usePortfolioHistory) as performance-chart.tsx, but presented bigger and
 * uncarded as the page's centerpiece rather than one card among several,
 * matching the reference's "Performance" section. Kept as its own
 * component (not a restyle of PerformanceChart) so the public /u/[address]
 * profile page, which reuses PerformanceChart, is untouched.
 */
export function PortfolioPerformanceHero({ address, subaccountName }: { address: string; subaccountName: string }) {
  const history = usePortfolioHistory(address, subaccountName);
  const [period, setPeriod] = useState<IndexerPortfolioPeriod>("week");
  const [hoverIndex, setHoverIndex] = useState<number | undefined>(undefined);

  const points = useMemo(() => history.data?.[period]?.accountValueHistory ?? [], [history.data, period]);
  ...
  const { path, areaPath, coords } = useMemo(() => {
    ...
    const coords = points.map((p, i) => {
      ...
      const y = range.isZero()
        ? CHART_HEIGHT / 2
        : CHART_HEIGHT - PADDING_Y - p.value.minus(min).div(range).toNumber() * (CHART_HEIGHT - PADDING_Y * 2);
      return { x, y };
    });
  ...
  const shown = hoverPoint ?? points[points.length - 1];
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-6 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.05),inset_0_-1px_0_0_rgba(0,0,0,0.2),0_16px_32px_-18px_rgba(0,0,0,0.7)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold text-foreground-muted">Performance</h2>
          {shown && <div className="mt-1 text-3xl font-semibold text-foreground">{formatUsd(shown.value)}</div>}
          {delta !== undefined && !hoverPoint && (
          ...
          {PERIODS.map((p) => (
            <button
              ...
              className={`rounded-full px-3 py-1 text-xs font-medium transition ${
                period === p.period ? "bg-surface-raised text-foreground" : "text-foreground-muted hover:text-foreground"
              }`}
            >
              {p.label}
            </button>
      ...
      {history.data && points.length === 0 && (
        <p className="mt-4 text-sm text-foreground-muted">No portfolio history yet for this period.</p>
      )}
      ...
      {points.length > 0 && (
        <div className="relative mt-4">
          ...
          >
            ...
            <path d={path} fill="none" stroke={seriesColor} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
            <circle cx={(hoverCoord ?? endCoord).x} cy={(hoverCoord ?? endCoord).y} r={4} fill={seriesColor} stroke="var(--surface)" strokeWidth={2} />
            {hoverCoord && <line x1={hoverCoord.x} x2={hoverCoord.x} y1={0} y2={CHART_HEIGHT} stroke="var(--border)" strokeWidth={1} />}
          </svg>
          {hoverPoint && (
            <div className="pointer-events-none absolute right-0 top-0 text-right text-xs text-foreground-muted">
